...
/**
 * The first step of every recipes trail, after the site's home: what stands at the prefix (§5.7).
 *
 * The index when it is on. When it is off the address belongs to somebody else — usually a page
 * made of blocks with the catalogue on it (decision 16) — and the trail starts with whatever the
...
final class Trail
{
    ...
    public static function index(string $locale): array
    {
        ...
        if (! (bool) $config->get('webx-recipes.index', true)) {
            return self::standIn($prefix, $locale);
        }

        return [new Crumb(
            (string) $container->make('translator')->get('webx-recipes::site.title', [], $locale),
            $container->make(SiteUrl::class)->to($prefix, $locale),
        )];
    }

    /**
     * Where "all recipes" leads: the last step of {@see index()}, or null when nothing answers.
     */
