 ...
 *
 * Three pages and one rule: the site names a view in `webx-services.views`, and until it has
 * written that view the package's bare one is used. A fresh installation therefore serves a
 * catalogue rather than "View [services.service] not found".
 */
final class Views
{
    /** What is printed when the site has written nothing of its own. */
    public const FALLBACK = 'webx-services::';
    ...
    {
        $configured = (string) $this->config->get("webx-services.views.{$which}", '');
