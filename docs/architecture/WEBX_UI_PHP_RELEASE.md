...
## Проверка в настоящем приложении

`scripts/php-smoke.sh` ставит пакеты в свежее приложение Laravel, мигрирует, заводит
администратора, входит по HTTP с настоящим CSRF-токеном и повторяет проверки с включёнными
`config:cache` и `route:cache`. В CI это job `Install into a real Laravel app` против MariaDB,
локально — тот же скрипт на sqlite.

Он ловит ровно то, что не может поймать Testbench: объявление провайдера для автодискавери,
поведение миграций на настоящей СУБД, CSRF (в тестах Laravel его отключает) и главное —
переживает ли кеш конфига мутация `webx-admin.api_middleware`, которую делает `module-auth`.
Если не переживёт, панель будет открыта в проде и закрыта везде, где смотрят.
...
- **Шаг тега стоит до `changesets/action`, и это не косметика.** Когда changesets есть что
  делать, он бампает версию и коммитит её: после него ни рабочее дерево, ни `HEAD` уже не то,
  что лежит на `main`. Тег, поставленный после, помечает релиз, которого никто не мержил, — и
  на коммите, который может вообще не доехать до `main` (так и вышло: ветку релиза бот
  пересоздал, а тег остался на осиротевшем коммите). `git show HEAD:` от этого не спасает.
