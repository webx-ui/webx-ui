...
#### Итог R4 — сделано 24.09.2026

Выпущено в v0.40.0 (#279, релизный #280): `webx-ui/module-reviews` на Packagist,
`@webx-ui/module-reviews@0.1.0` на npm (первая версия — руками из тарбола, снятого с
`changeset-release/main`; диапазоны `^0.33.2`/`^0.17.0`/`^0.6.1` на ядро, каркас и схему проверены
до публикации), `module-blocks@0.8.8`. Гейты npm и php (8.4) зелёные, `scripts/php-smoke.sh`
ставит модуль рядом с FAQ и прошёл против MariaDB 11.8 обеими половинами. Стоит на
`webx-cms.local` (сейчас в local-режиме на чекауте), на хомлабе и на `omnivitality-v2.local` (без
демо — у сайта нет демо-содержимого).

- **Очередь мержа через `gh pr merge` не работает** ни с `--squash`, ни без флага: gh пытается
  включить auto-merge, которого у репозитория нет. Ставит мутация `enqueuePullRequest` — CLAUDE.md §5.
- **Релизный PR без approve — подтверждено**: открыт от `app/webx-ui-split`, CI стартовал сам и
  шёл лёгким путём.
- **Демо на обоих сайтах — тинкером со своим журналом** (`storage/app/reviews-demo-journal.json`),
  как у FAQ: восемь отзывов, две категории, страница `/reviews`. Услуги сайта — стоматология, а не
  демо-набор, поэтому `ReviewsDemo` блок в услугу не ставит; слайдер «Сайты» добавлен в
  «Имплантацию под ключ» (id 1) тем же скриптом и записан в тот же журнал.
- Проверено живьём: сетка в три колонки без горизонтального скролла, фильтр категорий, звёзды,
  инициалы; слайдер — кнопки и автопрокрутка; бегущая строка на временно переключённом блоке
  `/reviews` (колонка переписана и возвращена байт в байт, без версии); Давид (текст только
  по-английски) на `/ru` не виден, на `/en` виден; `Review` и `AggregateRating` в `<head>` нет
  нигде. `prefers-reduced-motion` проверен по коду (`still.matches` у автопрокрутки и бегущей
  строки) — эмулировать медиазапрос панели браузера нечем. На хомлабе автопрокрутку увидеть не
  удалось: панель браузера была скрыта (`document.hidden`), а скрипт — тот же файл, что локально.
- **Открытый вопрос — Софья на `/ru` без имени и без инициалов.** Карточка берёт имя «на языке
  страницы, иначе на языке по умолчанию» (§4.3), а у Софьи имя только английское, текст — на обоих.
  На сайте с русским по умолчанию она видна пустым кружком и пустой подписью; в панели — `#4`.
  Итог R3 обещал «видна под английским именем» — это верно только там, где по умолчанию
  английский. Варианты: имя падает на любой язык, где оно есть (имя человека почти не
  переводится); или отзыв без имени на языке не виден вовсе, как без текста. Решает пользователь.
- Телефон — список с перетаскиванием и форма — за пользователем.
