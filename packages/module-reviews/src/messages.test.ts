...
const lang = (group: string) =>
  resolve(here, '../../../php/packages/module-reviews/lang/en', `${group}.php`)
...
describe('the English here matches the English the server ships', () => {
  it.each(['module', 'review', 'screen', 'category'])('%s', (group) => {
    const ours = Object.keys(reviewsMessages[group] ?? {}).sort()
