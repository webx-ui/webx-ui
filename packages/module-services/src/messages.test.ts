...
const lang = (group: string) =>
  resolve(here, '../../../php/packages/module-services/lang/en', `${group}.php`)
...
describe('the English here matches the English the server ships', () => {
  it.each(['module', 'panel', 'service', 'category'])('%s', (group) => {
    const ours = Object.keys(servicesMessages[group] ?? {}).sort()
