...
/**
 * A new event: what it is called, and nothing else. The server makes the address out of the
 * title; the address field on the settings tab is where there is room to say what changing it
 * costs.
 */
const { open, resolve, dismiss } = useModal<EventRow>()
...
async function submit(): Promise<void> {
  ...
  try {
    resolve((await api.create({ title: title.value.trim() })).event)
  } catch (error) {
