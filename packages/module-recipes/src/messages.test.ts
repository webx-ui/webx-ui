...
const lang = (group: string) =>
  resolve(here, '../../../php/packages/module-recipes/lang/en', `${group}.php`)
...
describe('the English here matches the English the server ships', () => {
  it.each(['module', 'panel', 'recipe', 'category', 'nutrient'])('%s', (group) => {
    const ours = Object.keys(recipesMessages[group] ?? {}).sort()
