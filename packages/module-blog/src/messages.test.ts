...
import { blogMessages } from './messages'

const here = dirname(fileURLToPath(import.meta.url))

const lang = (group: string) =>
  resolve(here, '../../../php/packages/module-blog/lang/en', `${group}.php`)
 ...
 * everywhere with one English word in the middle of it — and nothing else notices it, because a
 * missing key is not there to be compared.
 */
describe('the English here matches the English the server ships', () => {
  it.each(['module', 'panel'])('%s', (group) => {
