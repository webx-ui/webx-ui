...
- **Ручная публикация замораживает диапазоны на версиях «до релиза».** `workspace:^` подставляется
  ...
  резолвится в чекаут. Поэтому после релизного PR надо перевыпустить пакет патчем (и проверять
  `npm view <пакет>@<версия> dependencies`, а не package.json в репозитории).
