...
`WebxUi`), они же ядро админки на Laravel. **Имена пакетов на обеих половинах:** раздел панели —
с префиксом `module-` (`module-auth`, `module-media`), всё остальное — без него (`admin` —
каркас, `nested-set`, `localization`, `mcp` — библиотеки). Идентификатор модуля — то, чем он
называется в панели, а не то, что он хранит: администраторы — `admins`, а `users` оставлено
пользователям сайта, которые станут отдельным разделом. Реестр и планы — в
